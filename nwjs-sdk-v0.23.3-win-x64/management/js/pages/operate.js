    ...
    methods:{
        ...
        createWebScoket:function() {
            ...
            if ("WebSocket" in window)
            {
                ...
                {
                    var data = JSON.parse(evt.data)['hq_'+that.contractCode];
                    if(data){
                        that.hq = {
                            currentPrice:data.current_price,//当前价
                            downPrice:data.down_price,//跌停价
        ...
        buyOrSell:function (action,price) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: JSON.stringify({client_id:that.clientId,contract_code:that.contractCode,exchange_code:that.exchangeCodes,price:that.inputPrice,quantity:that.inputNumber,action:action,order_type:1}),
                dataType: "json",
                headers:global.netHeader,
                success: function (rsp, status, xhr) {
                    if(rsp.head.code == 0) {
